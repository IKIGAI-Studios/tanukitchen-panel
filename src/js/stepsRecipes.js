...
import { recipeAssistant } from "./class/recipeAssistant.js";
...
// Divide la URL en segmentos utilizando "/" como delimitador y obtén el último segmento
const segments = currentUrl.split("/");
const recipeId = segments[segments.length - 1];

let app = new recipeAssistant(
	recipeId,
	"switchStove",
	"switchScale",
	"switchSmoke",
	"label_scale_temp_goal",
	"label_stove_temp_goal",
	"btn_inst_ready",
	socketModule,
	socket
);
...
// fetch("/api/modules/getRecipe/" + recipeId)
...
const readCommand = (mensaje) => {
	...
	if (
		mensaje.includes("instruction ready") ||
		mensaje.includes("instrucción completa")
	) {
...
function setGoal() {
	...
	switch (recipe.steps[stepNum].instructions[instNum - 1].module) {
		case "scale":
			...
			$("#scale_goal").text(
				"Goal: " +
					recipe.steps[stepNum].instructions[instNum - 1].goal +
					" gr"
			);
...
function setTempTarget(tmp) {
	...
	fetch(url)
		...
		.then((json) => {
			json
				? bsAlert(
						`Temperature target settled in: <strong>${tmp}</strong> °`,
						"info"
				  )
				: bsAlert(
						`The temperature target <strong>couldn't</strong> be settled`,
						"danger"
				  );
		});
}
...
function readyInstruction() {
	...
	if (finished) window.location.replace("/recipes");
		...
		if (instNum != recipe.steps[stepNum].instructions.length) {
			...
			$("#inst_" + instNum).html(
				"<strike>" + $("#inst_" + instNum).text() + "</strike>"
			);
			instNum++;
			// Cambiar texto btn
			...
			json.map((module) => {
				switch (module.name) {
					case "extractor":
						...
						$("#smoke_turn").html(
							module.active ? "Turn Off" : "Turn On"
						);
					...
					case "smoke_detector":
						...
						if (
							module.values[0].value >= 30 &&
							module.values[0].value <= 60
						) {
					...
					case "scale":
						...
						$("#scale_weight").html(
							(module.active ? scale_weight : 0) + " gr"
						);
						break;
					case "stove":
...
function evaluateGoals() {
	if (!peso_correcto) {
		if (scale_goal_exist) {
			...
			if (
				scale_weight >=
					recipe.steps[stepNum].instructions[instNum - 1].goal - 10 &&
				scale_weight <=
					recipe.steps[stepNum].instructions[instNum - 1].goal + 10
			) {
...
function state_change(module, state) {
	...
	fetch(url)
		...
		.then((json) => {
			json
				? bsAlert(
						`The ${module} has been turned <strong>${
							state ? "on" : "off"
						}</strong> successfuly`,
						"primary"
				  )
				: bsAlert(
						`The ${module} <strong>couldn't</strong> be turned  ${
							state ? "on" : "off"
						}`,
						"danger"
				  );
