...
routes.get("/getModules/", async (req, res) => {
	try {
		if (req.session && req.session.user) {
			res.json(
				await Module.find({ id_kitchen: req.session.user.kitchen.name })
			);
		} else res.json(false);
	} catch (e) {
...
routes.get("/getScaleReg", async (req, res) => {
	try {
		if (req.session && req.session.user) {
			res.json(
				await scaleReg.find({ kitchen: req.session.user.kitchen.name })
			);
		} else res.json(false);
	} catch (e) {
		console.error(`Error: ${e}`);
	...
	try {
		res.json(
			await Module.aggregate([
				...
				}, // filtrar por el nombre del objeto
				{ $unwind: "$values" }, // desagregar el arreglo values
				{
					$group: {
						_id: null,
						averageValue: { $avg: "$values.value" },
					},
				},
			])
		);
